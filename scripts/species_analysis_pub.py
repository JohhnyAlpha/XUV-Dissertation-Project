...
# --------------------------------------------------
# SELECTING BEST CASES
# --------------------------------------------------
def choose_preferred_scale_for_scenario(star_dict, scenario):
    preferred = SCENARIO_TO_FSCALE.get(str(scenario))
    if preferred in star_dict and str(scenario) in star_dict[preferred]:
        return preferred

    for scale in sorted(star_dict.keys(), key=numeric_sort_key):
        if str(scenario) in star_dict[scale]:
            return scale
    return None
    ...
    for run in RUN_ORDER:
        ...
        for star in STAR_ORDER:
            ...
            for scenario in SCENARIO_ORDER:
                scale = choose_preferred_scale_for_scenario(star_dict, scenario)
                if scale is None:
                    continue
                entry = star_dict[scale].get(scenario)
...
def build_comparison_tables(metrics_df):
    ...
    for star in STAR_ORDER:
        ...
        for sp in SPECIES:
            ...
            # Across fscale at fixed run relative to 1.00
            for run in RUN_ORDER:
    ...
    return out.sort_values(["comparison_type", "star", "species", "run", "fscale", "metric"]).reset_index(drop=True)
...
def plot_case_species(case, out_dir):
    ...
    ax.set_title(f"{case['star']} | {case['run_label']} | FSCALE = {case['fscale']}")
    ...
    stem = os.path.join(
        out_dir,
        "case_species",
        f"species_{sanitize_for_filename(case['star'])}_{sanitize_for_filename(case['run'])}_fscale_{sanitize_for_filename(case['fscale'])}",
    )
...
# --------------------------------------------------
def heatmap_matrix(comparison_df, star, comparison_type, metric, value_selector=None):
    sdf = comparison_df[
...
def plot_heatmaps(comparison_df, out_dir):
    ...
    for star in STAR_ORDER:
        for comparison_type, title_stub in [
            ("vs_Run1_same_fscale", "relative to Run_1 at fixed FSCALE"),
            ("vs_Fscale1.00_same_run", "relative to FSCALE = 1.00 at fixed run"),
        ]:
            ...
            for ax, metric in zip(axes, metrics):
                mat = heatmap_matrix(comparison_df, star, comparison_type, metric)
            ...
            fig.suptitle(f"{star} | {title_stub}", y=0.98)
            stem = os.path.join(out_dir, "heatmaps", f"{sanitize_for_filename(star)}_{sanitize_for_filename(comparison_type)}")
            save_current_figure_both(fig, stem)
...
def plot_metric_scatter(metrics_df, out_dir):
    ...
    for star in STAR_ORDER:
        ...
        for ax, (metric, ylabel) in zip(axes, plot_specs):
            ...
            if metric != "peak_alt_km":
                finite_positive = metrics_df[metric].to_numpy(dtype=float)
        ...
        fig.suptitle(f"{star} | species metrics vs FSCALE", y=0.98)
        fig.tight_layout(rect=[0, 0.05, 1, 0.96])
        stem = os.path.join(out_dir, "metric_scatter", f"{sanitize_for_filename(star)}_metric_scatter")
        fig.savefig(stem + ".png", dpi=SAVE_DPI, bbox_inches="tight")
        fig.savefig(stem + ".pdf", bbox_inches="tight")
        print(f"Saved: {stem}.png")
        print(f"Saved: {stem}.pdf")
