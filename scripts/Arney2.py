...
# -----------------------------
# PUBLICATION-READY CASE SUMMARIES
# -----------------------------
def build_case_summary_figure(run, star, scale, scenario, entry):
    # Use constrained_layout to avoid tight_layout warnings for this complex figure
    ...
    # Top row: FTIR and FTSO spectra
    ax_ftir = fig.add_subplot(gs[0, 0])
    ax_ftso = fig.add_subplot(gs[0, 1])

    for ax, key, title, ylab in [
        (ax_ftir, "ftir", "FTIR Spectrum", "FTIR Flux"),
        (ax_ftso, "ftso", "FTSO Spectrum", "FTSO Flux"),
    ]:
        spec = entry.get(key)
        if spec is None:
        ...
        ax.plot(wavelength[mask], flux[mask], linewidth=1.2)
        ax.set_title(title)
        ax.set_xlabel(f"Wavelength ({spec['units']})")
        ax.set_ylabel(ylab)
        ax.grid(True, alpha=0.3)
    ...
    # Middle row: species vertical profiles across both columns
    ax_species = fig.add_subplot(gs[1, :])
    species_data = entry.get("species")

    if species_data is not None and "ALT" in species_data:
        ...
        for sp in PLOT_SPECIES:
            if sp in species_data:
                ...
                if np.any(mask):
                    ax_species.plot(x[mask], alt[mask], label=sp, linewidth=1.2)
                    plotted = True

        if plotted:
            ax_species.set_xscale("log")
            ax_species.legend(fontsize=8, ncol=4, loc="best")
        else:
            ax_species.text(0.5, 0.5, "No species data", ha="center", va="center", transform=ax_species.transAxes)

        ax_species.set_xlabel("Mixing Ratio")
        ax_species.set_ylabel("Altitude")
        ax_species.set_title("Atmospheric Composition")
    ...
    # Bottom left: integrated band totals
    ax_bands = fig.add_subplot(gs[2, 0])
    labels = ["UV", "VIS", "IR"]
    x = np.arange(len(labels))
    width = 0.38

    ftir_vals = entry.get("ftir", {}).get("integrated", {}) if entry.get("ftir") else {}
    ftso_vals = entry.get("ftso", {}).get("integrated", {}) if entry.get("ftso") else {}

    ftir_heights = [ftir_vals.get(k, np.nan) for k in labels]
    ftso_heights = [ftso_vals.get(k, np.nan) for k in labels]

    if np.any(np.isfinite(ftir_heights)) or np.any(np.isfinite(ftso_heights)):
        ax_bands.bar(x - width / 2, ftir_heights, width=width, label="FTIR")
        ax_bands.bar(x + width / 2, ftso_heights, width=width, label="FTSO")
        ax_bands.legend(fontsize=8)
    else:
        ax_bands.text(0.5, 0.5, "No band-integrated data", ha="center", va="center", transform=ax_bands.transAxes)

    ax_bands.set_xticks(x)
    ax_bands.set_xticklabels(labels)
    ax_bands.set_ylabel("Integrated Flux")
    ax_bands.set_title("Spectral Band Totals")
    ...
    if species_data is not None and "ALT" in species_data:
        present_species = [sp for sp in PLOT_SPECIES if sp in species_data]
        if present_species:
            summary_lines.append("Species available:")
            summary_lines.extend([f"  • {sp}" for sp in present_species])
            summary_lines.append("")

    for spec_key, label in [("ftir", "FTIR"), ("ftso", "FTSO")]:
        spec = entry.get(spec_key)
        if spec is None:
            summary_lines.append(f"{label}: missing")
            continue

        integ = spec.get("integrated", {})
        units = spec.get("units", "unknown")
        total = integ.get("TOTAL", np.nan)

        summary_lines.append(f"{label}:")
        summary_lines.append(f"  • Units: {units}")
        if np.isfinite(total):
            summary_lines.append(f"  • Total integrated flux: {total:.4e}")
        for band in ["UV", "VIS", "IR"]:
            val = integ.get(band, np.nan)
            if np.isfinite(val):
                summary_lines.append(f"  • {band}: {val:.4e}")
        summary_lines.append("")
    ...
    fig.suptitle(
        f"Publication Summary | {run} | {star} | scale={scale} | scenario={scenario}",
        fontsize=14
    )
    return fig


def save_case_summary_pngs(dataset, out_dir):
    for run in dataset:
        for star in dataset[run]:
            for scale in dataset[run][star]:
                for scenario, entry in dataset[run][star][scale].items():
                    print(f"\nSaving summary PNG: {run} | {star} | {scale} | scenario {scenario}")

                    fig = build_case_summary_figure(run, star, scale, scenario, entry)

                    filename = (
                        f"summary_{sanitize_for_filename(run)}_"
                        f"{sanitize_for_filename(star)}_"
                        f"scale_{sanitize_for_filename(scale)}_"
                        f"scenario_{sanitize_for_filename(scenario)}.png"
                    )
                    out_path = os.path.join(out_dir, "publication_summaries_png", filename)

                    ensure_dir(os.path.dirname(out_path))
                    fig.savefig(out_path, dpi=SUMMARY_DPI, format="png")
                    print(f"Saved: {out_path}")
...
def save_case_summary_pdfs(dataset, out_dir):
    all_stars = sorted({star for run in dataset for star in dataset[run]})

    for star in all_stars:
        pdf_path = os.path.join(
            out_dir,
            "publication_summaries_pdf",
            f"publication_summary_{sanitize_for_filename(star)}.pdf"
        )
        ensure_dir(os.path.dirname(pdf_path))

        print(f"\nSaving summary PDF for star: {star}")
        with PdfPages(pdf_path) as pdf:
            for run in dataset:
                ...
                for scale in dataset[run][star]:
                    for scenario, entry in dataset[run][star][scale].items():
                        fig = build_case_summary_figure(run, star, scale, scenario, entry)
                        pdf.savefig(fig, dpi=SUMMARY_DPI)
                        plt.close(fig)
...
if __name__ == "__main__":
    ...
    # New publication-ready summaries
    save_case_summary_pngs(data, PLOTS_DIR)
    save_case_summary_pdfs(data, PLOTS_DIR)

    print("\nDone.")
    print(f"JPEG plots written to: {PLOTS_DIR}")
    print(f"PNG summaries written to: {os.path.join(PLOTS_DIR, 'publication_summaries_png')}")
    print(f"PDF summaries written to: {os.path.join(PLOTS_DIR, 'publication_summaries_pdf')}")
