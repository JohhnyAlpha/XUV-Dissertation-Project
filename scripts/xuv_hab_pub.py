...
def should_use_log(arr, ratio_threshold=50):
    arr = valid_positive(arr)
    if arr.size < 2:
        return False
    return (np.nanmax(arr) / np.nanmin(arr)) > ratio_threshold
...
def apply_clean_axes_style(ax):
    ax.grid(True, alpha=0.25)
    for spine in ax.spines.values():
        spine.set_alpha(0.7)


def build_star_legend_handles(stars_in_df):
    handles = []
    for star in sorted(stars_in_df):
        handles.append(
            Line2D(
                [0], [0],
                marker="o",
...
def build_scenario_legend_handles(scenarios_in_df):
    handles = []
    for scenario in sorted(scenarios_in_df):
        handles.append(
            Line2D(
                [0], [0],
...
def add_dual_legend(ax, df):
    star_handles = build_star_legend_handles(df["star"].dropna().unique())
    scenario_handles = build_scenario_legend_handles(df["scenario"].dropna().unique())
        ...
        for star_id in sorted(os.listdir(run_dir), key=numeric_sort_key):
            ...
            for scale in sorted(os.listdir(star_dir), key=numeric_sort_key):
                ...
                for scenario, files in sorted(indexed.items(), key=lambda x: int(x[0])):
                    ...
                    surface_h2o_species = get_species_value(species_df, "H2O", "surface")
                    surface_o3_species = get_species_value(species_df, "O3", "surface")
                    upper_h2o_species = get_species_value(species_df, "H2O", "top")
                    upper_o3_species = get_species_value(species_df, "O3", "top")
                    ...
                    row = {
                        ...
                        "upper_h2o": (
                            upper_h2o_species
                            if np.isfinite(upper_h2o_species)
                        ...
                        "upper_o3": (
                            upper_o3_species
                            if np.isfinite(upper_o3_species)
...
def draw_trend_panel(ax, sub, x_col, y_col, x_label, y_label,
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if color_by_star:
            # used for all-star plots; color by star, marker by scenario
            for star in sorted(ssub["star"].dropna().unique()):
                sstar = ssub[ssub["star"] == star]
                xxs, yys = finite_xy(sstar[x_col], sstar[y_col])
                if len(xxs) == 0:
                    continue
                ax.scatter(
                    xxs, yys,
            ...
            star_name = ssub["star"].iloc[0] if not ssub.empty else None
            ax.scatter(
                xx, yy,
                marker=scenario_marker(scenario),
                s=62,
    ...
    if add_hline is not None:
        ax.axhline(add_hline, linestyle="--", color="black", linewidth=1.1)
    if add_vline is not None:
        ax.axvline(add_vline, linestyle="--", color="black", linewidth=1.1)

    ax.set_xlabel(x_label)
    ax.set_ylabel(y_label)
    apply_clean_axes_style(ax)
...
def plot_trend_by_star(df, x_col, y_col, title_prefix, y_label, out_stub,
    ...
    for star in stars:
        ...
        draw_trend_panel(
            ax, sub, x_col, y_col,
            x_label="XUV Proxy (scale)",
            y_label=y_label,
            y_log_candidate=y_log_candidate,
            add_hline=add_hline,
            add_vline=add_vline,
            color_by_star=False,
        )
        ...
        scenario_handles = build_scenario_legend_handles(sub["scenario"].dropna().unique())
        ax.legend(handles=scenario_handles, fontsize=8, loc="best", title="Scenario", title_fontsize=9)
...
def plot_o2_o3_dual_axis_by_star(df):
    ...
    for star in stars:
        ...
        ax1.axhline(ABIOTIC_O2_THRESHOLD, linestyle="--", color="black", linewidth=1.1)
        ax2.axhline(ABIOTIC_O3_THRESHOLD, linestyle="--", color="black", linewidth=1.1)
...
def plot_false_biosignature_by_star(df):
    ...
    for star in stars:
        ...
        ax.axvline(ABIOTIC_O2_THRESHOLD, linestyle="--", color="black", linewidth=1.1)
        ax.axhline(ABIOTIC_O3_THRESHOLD, linestyle="--", color="black", linewidth=1.1)
        ...
        if np.all(xx[np.isfinite(xx)] > 0) and xx[np.isfinite(xx)].size > 1:
            if np.nanmax(xx[np.isfinite(xx)]) / np.nanmin(xx[np.isfinite(xx)]) > 50:
                ax.set_xscale("log")
        if np.all(yy[np.isfinite(yy)] > 0) and yy[np.isfinite(yy)].size > 1:
            if np.nanmax(yy[np.isfinite(yy)]) / np.nanmin(yy[np.isfinite(yy)]) > 50:
                ax.set_yscale("log")
...
# PUBLICATION OUTPUTS
# -----------------------------
def plot_publication_trend_all_stars(df, x_col, y_col, title, x_label, y_label,
                                     out_name, y_log_candidate=False,
                                     add_hline=None, add_vline=None):
    valid = df[np.isfinite(df[x_col]) & np.isfinite(df[y_col])].copy()
    if valid.empty:
        return

    fig, ax = plt.subplots(figsize=PUB_FIGSIZE, constrained_layout=True)
    draw_trend_panel(
        ax, valid, x_col, y_col,
        x_label=x_label,
        y_label=y_label,
        y_log_candidate=y_log_candidate,
        add_hline=add_hline,
    ...
    ax.set_title(title)
    add_dual_legend(ax, valid)

    out_path = os.path.join(OUT_DIR, "publication", out_name)
    save_figure(fig, out_path, dpi=PUB_DPI, fmt=os.path.splitext(out_name)[1].replace(".", ""), use_tight=False)


def plot_publication_cross_all_stars(df, x_col, y_col, title, x_label, y_label,
                                     out_name, x_log_candidate=False, y_log_candidate=False,
                                     add_hline=None, add_vline=None):
    valid = df[np.isfinite(df[x_col]) & np.isfinite(df[y_col])].copy()
    if valid.empty:
        return
    ...
    if x_log_candidate and should_use_log(valid[x_col].values):
        ax.set_xscale("log")
...
def add_summary_text_panel(ax, sub):
    ax.axis("off")

    n_cases = len(sub)
    n_valid_temp = int(np.isfinite(sub["surface_temperature"]).sum())
    n_valid_h2o = int(np.isfinite(sub["surface_h2o"]).sum())
    n_valid_o2 = int(np.isfinite(sub["surface_o2"]).sum())
    n_valid_o3 = int(np.isfinite(sub["surface_o3"]).sum())

    lines = [
        f"Cases: {n_cases}",
        f"Valid temperature: {n_valid_temp}",
        f"Valid surface H2O: {n_valid_h2o}",
        f"Valid surface O2: {n_valid_o2}",
        f"Valid surface O3: {n_valid_o3}",
        "",
        f"Median Tsurf: {np.nanmedian(sub['surface_temperature']):.2f} K" if n_valid_temp else "Median Tsurf: n/a",
        f"Median H2O: {np.nanmedian(sub['surface_h2o']):.3e}" if n_valid_h2o else "Median H2O: n/a",
        f"Median O2: {np.nanmedian(sub['surface_o2']):.3e}" if n_valid_o2 else "Median O2: n/a",
        f"Median O3: {np.nanmedian(sub['surface_o3']):.3e}" if n_valid_o3 else "Median O3: n/a",
        "",
        f"Freezing reference: {FREEZING_TEMPERATURE_K:.2f} K",
        f"Desiccation reference: {DESICCATION_H2O_THRESHOLD:.1e}",
        f"Abiotic O2 reference: {ABIOTIC_O2_THRESHOLD:.1e}",
        f"Abiotic O3 reference: {ABIOTIC_O3_THRESHOLD:.1e}",
    ]
...
def build_publication_star_summary_figure(sub, star):
    fig = plt.figure(figsize=PUB_SUMMARY_FIGSIZE, constrained_layout=True)
    gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.0])

    ax1 = fig.add_subplot(gs[0, 0])
    draw_trend_panel(
        ax1, sub, "scale", "surface_temperature",
        x_label="XUV Proxy (scale)",
        y_label="Surface Temperature [K]",
        y_log_candidate=False,
        add_hline=FREEZING_TEMPERATURE_K,
        color_by_star=False,
    )
    ax1.set_title("(a) Surface temperature vs XUV")

    ax2 = fig.add_subplot(gs[0, 1])
    draw_trend_panel(
        ax2, sub, "scale", "surface_h2o",
        x_label="XUV Proxy (scale)",
        y_label="Surface H2O Mixing Ratio",
        y_log_candidate=True,
        add_hline=DESICCATION_H2O_THRESHOLD,
    ...
    ax2.set_title("(b) Surface H2O vs XUV")

    ax3 = fig.add_subplot(gs[1, 0])
    draw_trend_panel(
        ax3, sub, "surface_o2", "surface_o3",
        x_label="Surface O2 Mixing Ratio",
        y_label="Surface O3 Mixing Ratio",
        y_log_candidate=True,
        add_hline=ABIOTIC_O3_THRESHOLD,
        add_vline=ABIOTIC_O2_THRESHOLD,
        color_by_star=False,
    )
    if should_use_log(sub["surface_o2"].values):
        ax3.set_xscale("log")
    ax3.set_title("(c) False biosignature space")

    ax4 = fig.add_subplot(gs[1, 1])
    add_summary_text_panel(ax4, sub)
    ax4.set_title("(d) Summary statistics", loc="left", fontsize=11)
    ...
    fig.suptitle(f"Publication Summary | {star}", fontsize=15)
    return fig


def build_publication_overview_figure(df):
    fig = plt.figure(figsize=(13, 10), constrained_layout=True)
    gs = fig.add_gridspec(2, 2)

    ax1 = fig.add_subplot(gs[0, 0])
    draw_trend_panel(
        ax1, df, "scale", "surface_temperature",
        x_label="XUV Proxy (scale)",
        y_label="Surface Temperature [K]",
        y_log_candidate=False,
        add_hline=FREEZING_TEMPERATURE_K,
    ...
    if should_use_log(df["surface_o2"].values):
        ax3.set_xscale("log")
    ax3.set_title("(c) False biosignature space")
    ...
    grouped = (
        ...
        .agg(
            cases=("star", "size"),
            median_surface_temperature=("surface_temperature", "median"),
            median_surface_h2o=("surface_h2o", "median"),
            median_surface_o2=("surface_o2", "median"),
            median_surface_o3=("surface_o3", "median"),
        )
        .reset_index()
    )
    ...
    for _, row in grouped.iterrows():
        cell_text.append([
            row["star"],
            int(row["cases"]),
            f"{row['median_surface_temperature']:.1f}" if np.isfinite(row["median_surface_temperature"]) else "n/a",
            f"{row['median_surface_h2o']:.2e}" if np.isfinite(row["median_surface_h2o"]) else "n/a",
            f"{row['median_surface_o2']:.2e}" if np.isfinite(row["median_surface_o2"]) else "n/a",
            f"{row['median_surface_o3']:.2e}" if np.isfinite(row["median_surface_o3"]) else "n/a",
        ])

    table = ax4.table(
        cellText=cell_text,
        colLabels=["Star", "Cases", "Median T", "Median H2O", "Median O2", "Median O3"],
        loc="center",
        cellLoc="center",
    )
    ...
    ax4.set_title("(d) Per-star summary table", fontsize=11)

    add_dual_legend(ax2, df)

    fig.suptitle("Publication Overview | XUV Trend Analysis", fontsize=16)
    return fig


def save_publication_outputs(df):
    pub_dir = os.path.join(OUT_DIR, "publication")
    ensure_dir(pub_dir)

    # All-star single-figure outputs
    plot_publication_trend_all_stars(
        df, "scale", "surface_temperature",
        "Surface Temperature vs XUV",
        "XUV Proxy (scale)",
        "Surface Temperature [K]",
        "surface_temperature_vs_xuv_all.png",
        y_log_candidate=False,
        add_hline=FREEZING_TEMPERATURE_K,
    )
    plot_publication_trend_all_stars(
        df, "scale", "surface_h2o",
        "Surface H2O vs XUV",
        "XUV Proxy (scale)",
        "Surface H2O Mixing Ratio",
        "surface_h2o_vs_xuv_all.png",
        y_log_candidate=True,
        add_hline=DESICCATION_H2O_THRESHOLD,
    )
    plot_publication_cross_all_stars(
        df, "surface_o2", "surface_o3",
        "False Biosignature Space (O2 vs O3)",
        "Surface O2 Mixing Ratio",
        "Surface O3 Mixing Ratio",
        "false_biosignature_space_all.png",
        x_log_candidate=True,
        y_log_candidate=True,
        add_hline=ABIOTIC_O3_THRESHOLD,
        add_vline=ABIOTIC_O2_THRESHOLD,
    )

    # All-star overview summary
    overview_png = os.path.join(pub_dir, "xuv_publication_overview.png")
    fig = build_publication_overview_figure(df)
    save_figure(fig, overview_png, dpi=PUB_DPI, fmt="png", use_tight=False)

    overview_pdf = os.path.join(pub_dir, "xuv_publication_overview.pdf")
    fig = build_publication_overview_figure(df)
    ...
    # Per-star publication summaries
    stars = sorted(df["star"].dropna().unique())
    for star in stars:
        sub = df[df["star"] == star].copy()
        if sub.empty:
        ...
        png_path = os.path.join(pub_dir, f"xuv_publication_summary_{sanitize_for_filename(star)}.png")
        fig = build_publication_star_summary_figure(sub, star)
        save_figure(fig, png_path, dpi=PUB_DPI, fmt="png", use_tight=False)

        pdf_path = os.path.join(pub_dir, f"xuv_publication_summary_{sanitize_for_filename(star)}.pdf")
        fig = build_publication_star_summary_figure(sub, star)
...
def save_summary_tables(df):
    ...
    grouped_star = (
        ...
        .agg(
            ...
            median_surface_o2=("surface_o2", "median"),
            median_surface_o3=("surface_o3", "median"),
            median_surface_ch4=("surface_ch4", "median"),
            median_surface_co2=("surface_co2", "median"),
        )
        .reset_index()
    )
    grouped_star_csv = os.path.join(OUT_DIR, "xuv_trend_star_summary.csv")
    grouped_star.to_csv(grouped_star_csv, index=False)
    print(f"Saved: {grouped_star_csv}")
