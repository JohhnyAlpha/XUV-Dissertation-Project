...
FIGSIZE_STAR_MULTIPANE = (8.6, 7.0)  # extra width reserves space for right-side legend
...
def plot_star_difference_multipane(cases, star, scenario, out_dir,
    ...
    fig.suptitle(
        f"{star.replace('_', ' ')} | scenario {scenario} | difference from {RUN_LABELS.get(ref_run, ref_run)}, FSCALE={ref_fscale}",
        y=0.985,
    )
    fig.tight_layout(rect=[0.06, 0.04, 0.84, 0.955])
    ...
    stem = os.path.join(
        out_dir,
        "star_difference",
        f"{sanitize_for_filename(star)}_scenario_{sanitize_for_filename(scenario)}_delta_profiles_vs_{sanitize_for_filename(ref_run)}_fscale_{sanitize_for_filename(ref_fscale)}",
    )
    save_current_figure_both(fig, stem)
