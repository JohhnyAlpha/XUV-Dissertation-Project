...
def save_tables(synth, out_dir):
    ...
    for metric in ["CH4_column", "haze_proxy", "redox_proxy"]:
        ...
        out_csv = os.path.join(out_dir, f"haze_summary_{sanitize(metric)}.csv")
...
def save_regime_diagram(df, out_dir):
    ...
    save_fig(fig, os.path.join(out_dir, "publication_haze_regime_diagram.png"))


def save_haze_only_interpretation(df, out_dir):
    ...
    for ax, star in zip(axes, STAR_ORDER):
        ...
        for run in RUN_ORDER:
            ...
            summary = (
                ...
                .agg(
                    mean_value=("haze_proxy", "mean"),
                    std_value=("haze_proxy", "std"),
                )
                .fillna(0.0)
                .sort_values("fscale")
            )
        ...
        ax.set_ylabel(star.replace("_", " "))
        ax.set_yscale("log")
        style_axis(ax)

    axes[-1].set_xlabel("FSCALE")
    ...
    fig.suptitle(
        "Haze proxy response to stellar flux scaling",
        y=0.98,
        fontsize=12,
    )
    ...
    save_fig(fig, os.path.join(out_dir, "publication_haze_only_interpretation.png"))


def save_constant_metric_summary(df, metric, label, out_dir):
    ...
    ax.set_ylabel(label)
    ax.set_title(f"{label} remains stable across stellar flux scaling")
    style_axis(ax)

    vals = positive_finite(df[metric])
    if len(vals) > 0:
        ax.set_yscale("log")

    ax.legend(frameon=False, title="Stellar age")

    fig.text(
        0.5,
        -0.02,
        "Near-horizontal behaviour indicates this diagnostic is effectively fixed by the shared planet/template setup.",
        ha="center",
        fontsize=8,
    )
    ...
    save_fig(fig, os.path.join(out_dir, f"publication_constant_{sanitize(metric)}.png"))
...
def save_star_comparison_updated(df, out_dir):
    ...
    save_fig(fig, os.path.join(out_dir, "publication_star_comparison.png"))
...
def save_fscale_sensitivity_updated(df, out_dir):
    ...
    save_fig(fig, os.path.join(out_dir, "publication_fscale_sensitivity.png"))
...
def build_summary_text(df):
    ...
    lines.append("- Flat CH4 and redox trends likely reflect the shared planet/template setup across the run grid.")
    lines.append("")
...
def main():
    ...
    print("Saving regime diagram...")
    save_regime_diagram(synth, OUT_DIR)

    print("Saving haze-only interpretation...")
    save_haze_only_interpretation(synth, OUT_DIR)

    print("Saving CH4 stability plot...")
    save_constant_metric_summary(
        synth,
        "CH4_column",
    ...
    print("Saving redox stability plot...")
    save_constant_metric_summary(
        synth,
        "redox_proxy",
        "Redox proxy",
    ...
    print("Saving star comparison...")
    save_star_comparison_updated(synth, OUT_DIR)

    print("Saving FSCALE sensitivity...")
